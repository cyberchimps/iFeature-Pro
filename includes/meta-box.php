...
function initialize_the_meta_boxes() {
	...
	$mb
		...
		->tab($themenamefull." NivoSlider Options")
			...
			->sliderhelp('', 'Need Help?', '')
		->tab($themenamefull." Content Slider Options")
			->select('content_slider_category', 'Custom Slide Category', '', array('options' => $content_slider_options) )
			->text('content_slider_height', 'Slider Height', '', array('std' => '300'))
			->text('content_slider_delay', 'Slider Delay Time (MS)', '', array('std' => '3500'))
			->select('content_slider_animation', 'Slider Animation Type', '', array('options' => array('Horizontal-Push (default)', 'Fade', 'Horizontal-Slide', 'Vertical-Slide')) )
			->select('content_slider_navigation_style', 'Slider Navigation Style', '', array('options' => array('Dots (default)', 'None')) )
			->checkbox('content_slider_arrows', 'Navigation Arrows', '', array('std' => 'on'))
			->checkbox('content_slider_wordthumb', 'WordThumb Image Resizing', '', array('std' => 'off'))
			->sliderhelp('', 'Need Help?', '')
		->tab("Product Options")
